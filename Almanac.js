...
export default async function(ctx) {
  ...
  const C = {
    ...
    cardBg: { light: '#F2F2F7', dark: '#1C1C1E' }, // 新增：卡片背景色，用于区分层级
    ...
    term: { light: '#00C7BE', dark: '#32D74B' },     
    holiday: { light: '#007AFF', dark: '#0A84FF' },
    pz: { light: '#AF52DE', dark: '#BF5AF2' }      // 新增：彭祖百忌专属紫/品红色
  };
  ...
  // 📆 核心算法：计算当日干支，用于推导冲煞与彭祖百忌
  const cycle = (Math.round((Date.UTC(Y, M-1, D) - Date.UTC(1900,0,31)) / 86400000) + 40) % 60;
  const dCycle = cycle < 0 ? cycle + 60 : cycle;
  const dGan = dCycle % 10;
  const dZhi = dCycle % 12;

  // 📖 新增：推算彭祖百忌 (O(1) 索引映射)
  const pzGan = ["甲不开仓财物耗散","乙不栽植千株不长","丙不修灶必见灾殃","丁不剃头头必生疮","戊不受田田主不祥","己不破券二比并亡","庚不经络织机虚张","辛不合酱主人不尝","壬不汲水更难提防","癸不词讼理弱敌强"];
  const pzZhi = ["子不问卜自惹灾殃","丑不冠带主不还乡","寅不祭祀神鬼不尝","卯不穿井水泉不香","辰不哭泣必主重丧","巳不远行财物伏藏","午不苫盖屋主更张","未不服药毒气入肠","申不安床鬼祟入房","酉不会客醉坐颠狂","戌不吃犬作怪上床","亥不嫁娶不利新郎"];
  const pengzuText = `${pzGan[dGan]} · ${pzZhi[dZhi]}`;
  ...
  const rawYi = getVal("yi", "Yi", "suit").replace(/\./g, " ").trim() || "祭祀 祈福 斋醮 沐浴";
  const rawJi = getVal("ji", "Ji", "avoid").replace(/\./g, " ").trim() || "动土 破土 掘井 开光";
  ...
  return {
    ...
    children: [
      ...
      { type: 'stack', direction: 'row', alignItems: 'center', gap: 6, children: [
          ...
          { type: 'spacer' },
          { type: 'stack', backgroundColor: C.cardBg, cornerRadius: 6, padding: [2, 6], direction: 'row', alignItems: 'center', gap: 3, children: [
              { type: 'image', src: 'sf-symbol:sparkles', color: C.gold, width: 10, height: 10 },
              { type: 'text', text: zodiac, font: { size: 11, weight: 'bold' }, textColor: C.sub }
          ]}
      ]},
      { type: 'spacer', length: 6 }, 
      
      // 第二层：农历与节气焦点 (高亮展现)
      { type: 'text', text: `${obj.gz}(${obj.ani})年 ${obj.cn} ${shichenStr}${obj.term ? ` · 今日${obj.term}` : ` · 当前${currentTerm}`}`, font: { size: 14, weight: 'bold' }, textColor: C.gold },
      { type: 'spacer', length: 8 }, 

      // 第三层：黄历宜忌与彭祖百忌 (数据容器区块化，增加视觉秩序)
      { type: 'stack', direction: 'column', backgroundColor: C.cardBg, cornerRadius: 8, padding: [8, 10], gap: 6, children: [
          createRow("checkmark.circle.fill", C.yi, "宜", rawYi),
          createRow("xmark.circle.fill", C.ji, "忌", rawJi),
          { type: 'stack', direction: 'row', alignItems: 'center', gap: 6, children: [
              { type: 'stack', direction: 'row', alignItems: 'center', gap: 3, children: [
                  { type: 'image', src: 'sf-symbol:scroll.fill', color: C.pz, width: 14, height: 14 },
                  { type: 'text', text: "忌", font: { size: 12, weight: 'heavy' }, textColor: C.pz }
              ]},
              { type: 'text', text: pengzuText, font: { size: 12, weight: 'bold' }, textColor: C.sub, maxLines: 1, flex: 1 }
          ]}
      ]},
      { type: 'spacer', length: 8 },

      // 第四层：运势/冲煞与节假日 (底部信息网格)
      { type: 'stack', direction: 'column', gap: 4, children: [
          { type: 'stack', direction: 'row', alignItems: 'center', gap: 6, children: [
              { type: 'image', src: 'sf-symbol:shield.lefthalf.filled', color: C.muted, width: 12, height: 12 },
              { type: 'text', text: "冲煞:", font: { size: 11, weight: 'bold' }, textColor: C.muted },
              { type: 'text', text: chongshaInfo, font: { size: 11, weight: 'medium' }, textColor: C.muted, maxLines: 1, flex: 1 },
              { type: 'text', text: starStr, font: { size: 10, weight: 'medium' }, textColor: C.muted }
          ]},
          { type: 'stack', direction: 'row', alignItems: 'center', gap: 4, children: [
              { type: 'image', src: 'sf-symbol:leaf.arrow.circlepath', color: C.term, width: 12, height: 12 }, 
              { type: 'text', text: "节气:", font: { size: 11, weight: 'bold' }, textColor: C.term }, 
              { type: 'text', text: upcomingTerms.join(", "), font: { size: 11, weight: 'medium' }, textColor: C.term, maxLines: 1, flex: 1 }
          ]},
          { type: 'stack', direction: 'row', alignItems: 'center', gap: 4, children: [
              { type: 'image', src: 'sf-symbol:flag.fill', color: C.holiday, width: 12, height: 12 },
              { type: 'text', text: "法定:", font: { size: 11, weight: 'bold' }, textColor: C.holiday }, 
